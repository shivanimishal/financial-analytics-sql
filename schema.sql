-- ============================================================
-- P2P Financial Analytics — Star Schema DDL
-- Author: Shivani Mishal
-- Design rationale: Star schema chosen over snowflake because:
--   (1) self-service analytics users need simple joins
--   (2) Power BI and Tableau both perform better against stars
--   (3) 100+ stakeholders with mixed SQL ability — simplicity wins
-- ============================================================
 
-- ── DIMENSION: VENDOR ───────────────────────────────────────
-- A dimension table holds descriptive attributes about an entity
-- vendor_id is the PRIMARY KEY — each vendor appears exactly once
-- SCD Type 1 applied: attributes overwrite on change (we don't need history)
CREATE TABLE dim_vendor (
    vendor_id           VARCHAR(10)   PRIMARY KEY,
    vendor_name         VARCHAR(200)  NOT NULL,
    category            VARCHAR(100),
    -- CHECK constraint enforces only valid region codes enter the table
    -- This is a data governance control — prevents 'ASIA' vs 'APAC' inconsistency
    region              VARCHAR(10)   CHECK (region IN ('APAC','EMEA','AMER')),
    payment_terms_days  INT,
    -- EPD eligibility flags which vendors are enrolled in the Early Payment programme
    is_epd_eligible     BOOLEAN       DEFAULT FALSE,
    platform            VARCHAR(100)
);
 
-- ── DIMENSION: DATE ─────────────────────────────────────────
-- A date dimension is standard in any BI schema
-- It enables time intelligence without expensive date functions at query time
-- Pre-computing year/quarter/month here means analysts don't repeat DATEPART() everywhere
CREATE TABLE dim_date (
    date_key            DATE          PRIMARY KEY,
    year                INT,
    quarter             INT,
    month               INT,
    month_name          VARCHAR(20),
    week_of_year        INT,
    -- is_month_end useful for month-end close reporting cycles
    is_month_end        BOOLEAN
);
 
-- ── FACT: INVOICE ────────────────────────────────────────────
-- A fact table holds measurable events — each invoice is one event (one row)
-- The grain is: one row per invoice
-- Foreign keys link back to dimension tables for joins
-- amount_usd and dates are the measures — what gets aggregated in reports
CREATE TABLE fact_invoice (
    invoice_id          VARCHAR(15)   PRIMARY KEY,
    -- REFERENCES enforces referential integrity — no orphan invoices without a vendor
    vendor_id           VARCHAR(10)   REFERENCES dim_vendor(vendor_id),
    platform            VARCHAR(100),
    invoice_date        DATE,
    due_date            DATE,
    -- paid_date is nullable — unpaid invoices have no paid date
    paid_date           DATE,
    -- DECIMAL(15,2): up to 15 digits with 2 decimal places — handles multi-million amounts
    amount_usd          DECIMAL(15,2),
    -- CHECK constraint restricts status to valid values only
    -- Prevents dirty data like 'PAID' (uppercase) vs 'Paid' co-existing
    status              VARCHAR(20)   CHECK (status IN
                        ('Paid','Approved','Pending','Disputed','Rejected')),
    region              VARCHAR(10),
    invoice_type        VARCHAR(30)
);
 
-- ── FACT: SLO COMPLIANCE LOG ─────────────────────────────────
-- Tracks every approval process — one row per process event
-- This table powers the SLO compliance reporting on your GS resume
-- (the metric that moved from 10-12 day average to the 7-day SLO)
CREATE TABLE fact_slo_log (
    process_id          VARCHAR(15)   PRIMARY KEY,
    region              VARCHAR(10),
    platform            VARCHAR(100),
    submitted_date      DATE,
    approved_date       DATE,
    days_to_approve     INT,
    -- DEFAULT 7: the SLO target — stored per row in case targets change by region
    slo_target_days     INT           DEFAULT 7,
    -- Computed flag: TRUE if approved within target, FALSE if breached
    slo_met             BOOLEAN,
    approver_type       VARCHAR(50)   -- Manager / Director / VP / MD
);